...
const executeQuery = async <T>(query: string, params: unknown[] = []): Promise<T> => {
  ...
  try {
    ...
    console.error('SQLite 쿼리 실행 오류:', error);
    throw new Error('데이터베이스 쿼리 실행 중 오류가 발생했습니다.');
  }
};
...
const executeQuerySingle = async <T>(query: string, params: unknown[] = []): Promise<T | null> => {
  ...
  try {
    ...
    console.error('SQLite 단일 쿼리 실행 오류:', error);
    throw new Error('데이터베이스 쿼리 실행 중 오류가 발생했습니다.');
  }
};
...
const executeRun = async (query: string, params: unknown[] = []): Promise<{ lastID: number; changes: number }> => {
  ...
  try {
    ...
    return {
      lastID: (result.lastInsertRowid as number) || 0,
      changes: result.changes || 0,
    };
  } catch (error) {
    console.error('SQLite 변경 쿼리 실행 오류:', error);
    throw new Error('데이터베이스 변경 쿼리 실행 중 오류가 발생했습니다.');
  }
};
 ...
 * 트랜잭션 실행 함수
 */
const executeTransaction = async <T>(callback: (db: Database.Database) => Promise<T>): Promise<T> => {
  const db = await getDbConnection();
  ...
  try {
    ...
    throw new Error('트랜잭션 실행 중 오류가 발생했습니다.');
  }
};
